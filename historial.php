...
function obtenerHistorial($userId){	
	...
	while ($resultado = mysqli_fetch_assoc($res)) {		
		...
		$results[$resultado['fechaCompra']]['nombre'] = "<a href='".IS2_ROOT_PATH."/ofertas/".$resultado['id']."'>".$resultado['nombre']."</a>";
	...
	while ($resultado = mysqli_fetch_assoc($res)) {		
		...
		$results[$resultado['fecha']]['nombre'] = "<a href='".IS2_ROOT_PATH."/subastas/".$resultado['subasta']."'>".$name['nombre']."</a>";
